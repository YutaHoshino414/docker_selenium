...
class Crawler:
    ...
    def crawl_main(self):  
        ...
        sleep(2)
        for div in self.driver.find_elements_by_xpath('//div[@class="jobName"]/h2/a'):
            self.data['title'] = re.sub("！|／|＠|＊","/",div.text.replace(',',''))
            self.data['url'] = div.get_attribute('href')
            with open('./data/results.jsonlines', 'a') as f:
